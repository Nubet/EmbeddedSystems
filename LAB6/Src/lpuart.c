...
static void LPUART_Init_Pins(void) {
    ...
    lpuart_pins.Pull = PUDPR_PULL_NO;
...
int LPUART_SendChar(unsigned char data) {
    while (Read_Specific_32Register_Bit(&(MY_LPUART1->ISR), LPUART_ISR_TXE_Pos) == 0) {
...
int LPUART_ReceiveChar(unsigned char* data) {
    if (Read_Specific_32Register_Bit(&(MY_LPUART1->ISR), LPUART_ISR_RXNE_Pos) == 1) {
